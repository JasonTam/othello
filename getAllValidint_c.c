...
/* Flips peices */
static int rayFlip( int b[], int m, int n, int tok,
        int potB[], int (*dirFnPtr)(int, int, int), int (*bFnPtr)(int, int, int)) {
    int off = 1;
    int valid = 0;
...
static int isValid( int	b[], int m, int n, int tok,
        int potB[] ) {
    int valid = 0;
...
static int getAllValid( int	*nb, int *a,
           int b[], int tok, int *potB ) {
        ...
        for ( m = 0; m < L; m++) {
            ...
            if (b[i]==0) {          // Only analyze for empty spots
                ...
                if (dila) {     // Only analyze for spots touching enemies
                    ...
                    memcpy((void *)potB,(const void *)b,L*L*sizeof(int));             
                    ...
                    if (isValid(b,m,n,tok,potB)>0) { // If it's a valid move
                        ...
                        memcpy((void *)nb+(c++)*L*L*sizeof(int),(const void *)potB,L*L*sizeof(int));
                    }
                }
            }
        }
...
{ 
    int *nb,*a;      /* Output Vars */
    int *b,*tok;     /* Input Vars */
    size_t m,n; 
...
//     if (!mxIsInt8(B_IN) || mxIsComplex(B_IN) || 
// 	(m != L) || (n != L)) { 
...
//     } 
    ...
    b = (int *)mxGetData(B_IN); 
    tok = (int *)mxGetData(TOK_IN);
    ...
    NB_OUT = mxCreateNumericArray( NB_DIMS, dims, mxINT32_CLASS, mxREAL);
    A_OUT = mxCreateNumericMatrix( n_valid, 1, mxINT32_CLASS, mxREAL);
            
    /* Assign pointers to the output parameters */ 
    nb = (int *)mxGetData(NB_OUT);
    a = (int *)mxGetData(A_OUT);
    
    /* Copy over results from tmp variables to output */
    if (n_valid>0) {
        memcpy((void *)nb,(const void *)nb_temp,n_valid*L*L*sizeof(int));
        memcpy((void *)a,(const void *)a_temp,n_valid*sizeof(int));
    }
